...
and the ``\verb # '' indicates a comment in the bash language.
...
If you need help to get familliar with Linux (Unix), I suggest the first 6 tutorials of this website: 
...
Once you are familliar with Linux, you can connect to mammouth, the supercalculator at Sherbrooke University. 
Of course, you will need to get a username and a password with the help of your supervisor.
Let's suppose here that your supervisor is Andre-Marie Tremblay and that the username you got is \verb charleb1 .
...
\end{lstlisting}
\end{bashInput}
the ``-r '' (recursively) is to copy folders (like with the \verb cp  command). 
Your password will be asked like for \verb ssh .%, except if you make it automatic (next).
...
There is also a fancier and more powerful way, with the \verb bqsubmit  command (see section \ref{bqsubmit}). 
...
If you want to lauch job on MP, the most common queue is \verb qwork@mp2 . 
It is important to stress that you must be logged on MS when you \verb qsub  a ``pbs'' file with \verb qwork@ms  and on MP when you \verb qsub  a ``pbs'' file with \verb qwork@mp2 .
...
On MS, there is 8 cores per node and 16 Go of memory. 
...
Once on the node, you can, for example, check the CPU usage and the memory allocated with the command \verb top : 
...
A good practice is to use the \verb bin  directory.

In your home directory (the one you access by doing the \verb cd  alone, or \verb cd  \verb ~ ), you should have a directory named \verb bin .
...
To get a pretty Fermi surface, you can go to the directory \verb onebody  and run these commands:
\begin{bashInput}
\begin{lstlisting}[style=BashInputStyle]
